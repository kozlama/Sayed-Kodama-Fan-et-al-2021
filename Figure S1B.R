...
Gan_46 <- readRDS(file = "Gan_46_singlets_PCA.rds")
...
Gan_59 <- readRDS(file = "Gan_59_singlets_PCA.rds")
Gan_60 <- readRDS(file = "Gan_60_singlets_PCA.rds")
Gan_61 <- readRDS(file = "Gan_61_singlets_PCA.rds")
Gan_62 <- readRDS(file = "Gan_62_singlets_PCA.rds")
Gan_63 <- readRDS(file = "Gan_63_singlets_PCA.rds")
Gan_64 <- readRDS(file = "Gan_64_singlets_PCA.rds")
Gan_65 <- readRDS(file = "Gan_65_singlets_PCA.rds")
Gan_66 <- readRDS(file = "Gan_66_singlets_PCA.rds")

setwd("/athena/ganlab/scratch/lif4001/Human_AD_Mayo_UPenn/all_55/data_analysis/integration")
...
WT_E3E3_M <- c(Human_R47H_6, Human_R47H_10, Gan_46)
...
rm(Human_R47H_6, Human_R47H_10, WT_E3E3_M,Gan_46)
...
AD_all <- c(E2E4_integrated, E3E3_integrated, E3E4_integrated, E4E4_integrated)
anchors_AD_all <- FindIntegrationAnchors(object.list = AD_all, dims = 1:30)
AD_all_integrated <- IntegrateData(anchorset = anchors_AD_all, dims = 1:30)
rm(E2E4_integrated, E3E3_integrated, E3E4_integrated, E4E4_integrated, AD_all)
...
rm(Non_E2E3, Non_E2E3_F_integrated,Non_E2E3_M_integrated)
...
rm(Non_E3E3, Gan_65,Gan_66)

Non_all <- c(Non_E2E3_integrated,Non_E3E3_integrated,Gan_61, Gan_64)
anchors_Non_all <- FindIntegrationAnchors(object.list = Non_all, dims = 1:30)
Non_all_integrated <- IntegrateData(anchorset = anchors_Non_all, dims = 1:30)
rm(Non_all, Non_E2E3_integrated,Non_E3E3_integrated,Gan_61, Gan_64)

all <- c(AD_all_integrated, Non_all_integrated)
anchors_all <- FindIntegrationAnchors(object.list = all, dims = 1:30)
all_integrated <- IntegrateData(anchorset = anchors_all, dims = 1:30)
rm(all, AD_all_integrated, Non_all_integrated)

pdf("all_integrated_QC_1.pdf", width=16, height=12)
Idents(all_integrated) <- "orig.ident"
VlnPlot(object = all_integrated, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 1, pt.size=0, idents=NULL)
dev.off()
pdf("all_integrated_QC_2.pdf", width=16, height=4)
Idents(all_integrated) <- "Condition"
VlnPlot(object = all_integrated, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size=0, idents=NULL)
dev.off()

#saveRDS(all_integrated, file = "all_integrated.rds")
...
#all_integrated <- readRDS("all_integrated.rds")
...
#all_integrated <- FindVariableFeatures(all_integrated, selection.method = "vst", nfeatures = 3000)

all_integrated <- ScaleData(all_integrated, verbose = FALSE)
all_integrated <- RunPCA(all_integrated, features = VariableFeatures(object = all_integrated), verbose = FALSE)
...
all_integrated <- NormalizeData(all_integrated, normalization.method = "LogNormalize", scale.factor = 10000)
all_integrated <- ScaleData(all_integrated, features = rownames(all_integrated))

pdf("all_integrated_umap.pdf", width=6, height=4)
DimPlot(all_integrated, reduction = 'umap', label = T)
dev.off()
pdf("all_integrated_umap_split_individual.pdf", width=14, height=12)
DimPlot(all_integrated, reduction = "umap", split.by = "orig.ident", label = T, ncol = 6)
dev.off()
pdf("all_integrated_umap_split_Condition.pdf", width=6, height=12)
DimPlot(all_integrated, reduction = "umap", split.by = "Condition", label = T, ncol = 2)
dev.off()

saveRDS(all_integrated, file = 'all_integrated_PCA_0.1.rds')
...
#all_integrated <- readRDS("all_integrated_PCA_0.1.rds")

DefaultAssay(all_integrated) <- 'RNA'
pdf("all_integrated_umap_test.pdf", width=8, height=6)
DimPlot(all_integrated, reduction = 'umap', label = T)
dev.off()

#Add marker genes

pdf("all_integrated_annotation_combine.pdf", width=12, height=6)
sig_all<-c("SYT1","SNAP25","GRIN1","SLC17A7", "CAMK2A", "NRGN","GAD1", "GAD2", "PLP1", "MBP", "MOBP","AQP4", "GFAP", "CD74", "CSF1R", "C3","PDGFRA", "VCAN", "EBF1","IGFBP7","FLT1","CLDN5")
markers.to.plot <- as.matrix(sig_all)
DotPlot(object = all_integrated, features = rev(x = markers.to.plot)) + RotatedAxis()
dev.off()

all_integrated <- subset(all_integrated, idents = c("17"), invert = TRUE)
DefaultAssay(all_integrated) <- 'RNA'
all_integrated <- RenameIdents(all_integrated,
                               ...
                               `8`="excitatory neurons", `9`="inhibitory neurons", `10`="endothelial cells", `11`="excitatory neurons",
                               `12`="oligodendrocytes", `13`="inhibitory neurons", `14`="excitatory neurons",
                               `15`="excitatory neurons", `16` = "excitatory neurons", `18`="oligodendrocytes"
)

### Fig.S1B
pdf("all_integrated_umap_annotation.pdf", width=8, height=6)
DimPlot(all_integrated, reduction = 'umap', label = TRUE)
dev.off()


## Cell type composition bar plot
pdf("all_integrated_cell_counts.pdf", width=4, height=3)
cell_types <- ggplot(all_integrated@meta.data, aes(orig.ident))
cell_types + geom_bar(aes(fill = Condition)) +theme_minimal() + RotatedAxis()
dev.off()

write.csv(table(all_integrated@active.ident, all_integrated@meta.data$Condition), "all_cell_counts_ByCondition.csv")
write.csv(table(all_integrated@active.ident, all_integrated@meta.data$orig.ident), "all_cell_counts_BySample.csv")

pdf("all_integrated_cell_table.pdf", width=4, height=3)
cell_types <- ggplot(all_integrated@meta.data, aes(seurat_clusters))
cell_types + geom_bar(aes(fill = Condition)) +theme_minimal() + RotatedAxis()
cell_types + geom_bar(aes(fill = orig.ident)) +theme_minimal() + RotatedAxis()
dev.off()
